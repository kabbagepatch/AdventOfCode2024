...
int main () {
  ...
  for (int i = 0; i < lines.size(); i += 4) {
    ...
    long double p_x = 10000000000000 + stoi(prize.substr(PRIZE_PREFIX.size(), PRIZE_PREFIX.size() + prize_comma_ind));
    long double p_y = 10000000000000 + stoi(prize.substr(prize_comma_ind + 4));

    int det = a_x * b_y - a_y * b_x;
    if (det == 0) continue;

    long double a_solution = abs((b_x * p_y - p_x * b_y) / det);
    long double b_solution = abs((a_x * p_y - p_x * a_y) / det);
    if (long(a_solution) != a_solution || long(b_solution) != b_solution) continue;
