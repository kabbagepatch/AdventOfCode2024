...
bool isResultPossible(string operandString, long result) {
  ...
  int operandPos = to_string(result).rfind(to_string(lastOperand));
  if (operandPos != string::npos && operandPos > 0 && operandPos == to_string(result).length() - to_string(lastOperand).length()) {
    concatenation = isResultPossible(newOperandString, stol(to_string(result).substr(0, operandPos)));
  }
